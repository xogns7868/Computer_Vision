...
volatile unsigned long int passing_sec = 0;
volatile unsigned long int detect_object_sec = 0;
...
static void draw_operatingtime(struct display *disp, uint32_t time) //edge detect 수행시간
{
...
static void drive(struct display *disp, struct buffer *cambuf)
{
    ...
    if(get_framebuf(cambuf, cam_pbuf) == 0) {
      ...
      else if(mode == 7 && passing_lane_flag == 4 || passing_lane_flag == 5 || passing_lane_flag == 1){
...
static void trafficLightMission(struct display *disp, struct buffer *cambuf)
{
    ...
    if(get_framebuf(cambuf, cam_pbuf) == 0) {
        ...
        // MSG("trafficLightMission - traffic light flag = %d", traffic_light_flag);
        gettimeofday(&et, NULL);
        optime = ((et.tv_sec - st.tv_sec)*1000)+ ((int)et.tv_usec/1000 - (int)st.tv_usec/1000);
        draw_operatingtime(disp, optime);
    }
...
void * capture_thread(void *arg)
{
    ...
    switch(mode) {
      ...
      case 1 :  // 출발 및 도로주행
          ...
          driveOnOff = 1;
          cameraOnOff = 1;
          if(passing_lane_flag == 1 || passing_lane_flag == 2){
...
void mode_overpass()
{
  //고가도로 내리막길 들어가기 전에 정상속도 -> 내리막길 이후 속도 20
  if(time(NULL) - sec >=3 ){ 
    MSG("stop!");
    overpass_flag = 1;
    } // 고가도로 종료 조건
...
void mode_outbreak()
{
  ...
  // printf("red pixel count : %d\n", outbreak_count);
   MSG("realmode = %d", mode);
  MSG("outbreak_count: %d",outbreak_count);
  //돌발 장애물 미션 : 최초 인식
...
void horizontal_parking_right()       // 수평 주차 모드
{
  ...
  SteeringServoControl_Write(/*angle*/ 1400);
  usleep(1000000);
//   while(1){
//     dist = get_distance(5);
//     // printf("주차 %d CM\n" , dist);
...
void mode_rotary()
{
  if(SteeringServoControl_Read() < 1200){
    curve_count++;
  }
  MSG("%d, %d", stop_line_count, curve_count);
  // 정지선 인식 = 회전 교차로 시작전
  // printf("ROTARY_MODE !! \n");
  if(rotary_flag == 0){
    if(stop_line_flag == 0 && stop_line_count > 1000 && curve_count > 0){
      rotary_flag = 1;
      DesireSpeed_Write(50);
      usleep(1000000);
      SteeringServoControl_Write(1500);
...
void mode_passing_lane()
{
    ...
    usleep(3000000);
    passing_lane_flag = 1;
    detect_object_sec = time(NULL);
    }
    }
    else if(passing_lane_flag == 1){
		// printf("...판별중...\n");
    if(DistanceSensor(1) > 1500){
      passing_sec = time(NULL);
      passing_lane_flag = 2;
      }
    if(is_return){
      MSG("is_return");
      ...
      DesireSpeed_Write(-50);
      MSG("%d, %d", passing_sec,  passing_sec - detect_object_sec);
      SteeringServoControl_Write(1500);
      usleep((passing_sec - detect_object_sec)*600000);
      SteeringServoControl_Write(1900);
      DesireSpeed_Write(-50); // 원래  -50
      usleep((passing_sec - detect_object_sec)*1500000);
      //usleep(1500000);
      SteeringServoControl_Write(1000);
      DesireSpeed_Write(-50);
      usleep(2000000);
...
void left_rotate(){
  ...
  if(true){
    ...
    SteeringServoControl_Write((int) steer + 1500 - 50);
...
void right_rotate(){
  ...
  if(true){
    ...
    SteeringServoControl_Write( 1500 - (int)steer + 100); 
...
void mode_traffic_light(){
  //흰색 차선 발견시 정지
  if(stop_line_count > 1000 && traffic_light_flag < 0){
